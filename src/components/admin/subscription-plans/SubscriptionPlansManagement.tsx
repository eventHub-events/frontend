...
export default function SubscriptionPlansManagement() {
   ...
   const[stats, setStats] = useState({
    ...
    const handleCreate = () =>  {
     ...
     const handleSave = (data: ISubscriptionPlan) => {
          if(selectedPlan) {
            const updated = await subscriptionPlansService.update(selectedPlan.id!, data);
             setPlans((prev) => 
               prev.map((p) =>p.id === updated.id? updated: p)
             )
              toast.success("Plan updated successfully");
          }else{
            const created = await subscriptionPlansService.create(data);
   ...
   const fetchPlans = async () => {
       try{
           const res = await subscripionPlanService.getAll();
           setPlans(data)
     }catch(err){
          console.log(err)
       }finally{
    ...
    return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard icon={<Users />} label="Total Organizers" value={stats.totalOrganizers.toLocaleString()} />
        <StatCard icon={<DollarSign />} label="Monthly Revenue" value={`₹${stats.monthlyRevenue.toLocaleString()}`} />
        <StatCard icon={<TrendingUp />} label="ARPU" value={`₹${stats.arpu}`} />
        <StatCard icon={<Percent />} label="Churn Rate" value={`${stats.churnRate}%`} />
      </div> */}

      {/* Subscription Plans */}
      <div className="flex justify-between items-center mt-4">
        <h3 className="text-lg font-semibold">Subscription Plans</h3>
        <Button className="bg-red-500 hover:bg-red-600 text-white">+ Create Plan</Button>
      </div>
            ...
            <CardContent className="p-6 flex flex-col justify-between h-full">
              ...
              <div className="flex items-center justify-between mt-6">
                ...
                >
                 ...
                 <Button variant="outline" className="mr-2" onClick={() => handleEdit(plan)}>
                Edit
              </Button>
