...
import { FaCheckCircle, FaClock, FaCloudUploadAlt, FaTimesCircle } from "react-icons/fa";
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {documents.map((doc) => {
            ...
            return (
              <div
                key={doc._id}
                className="rounded-lg border border-gray-200 shadow-sm bg-white overflow-hidden hover:shadow-md transition duration-200 flex flex-col"
              >
                {/* Image */}
                ...
                <div className="p-4 flex justify-between items-start">
                 ...
                 <div className={`flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full ${status.bg} ${status.color}`}>
  {status.icon}
  {status.label}
</div>
                </div>
